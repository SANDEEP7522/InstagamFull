...
export const addNewPost = async (req, res) => {
  try {
    ...
    const post = await Post.create({
      ...
      author: authorId, // take author id
    });
    ...
    // Populate the author field correctly
...
export const getCommentPost = async (req, res) => {
  try {
    ...
    ); // method allows you to reference documents in other collections.
    if (!comments) {
      return res.status(404).json({
...
export const postBookmark = async (req, res) => {
  try {
    ...
    if (!post) {
      return res
        .status(404)
        .json({ message: "post not fount", success: false });
    }
